...
for batch in [32, 128, 256]:
    for seed in [420, 42, 402]:
        ...
        model, hist, loss, acc = create_model(seed, 50, batch)
        models.append(model)
        histories.append(hist)
        losses.append(loss)
        accs.append(acc)

print("Batch size 32 avg loss: {0:.4f}, avg accuracy: {1:.4f}".format(np.mean(losses[:3]), np.mean(accs[:3])))
print("Batch size 128 avg loss: {0:.4f}, avg accuracy: {1:.4f}".format(np.mean(losses[3:6]), np.mean(accs[3:6])))
print("Batch size 256 avg loss: {0:.4f}, avg accuracy: {1:.4f}".format(np.mean(losses[6:]), np.mean(accs[6:])))

df = pd.DataFrame({"loss": losses, "acc": accs})
df.to_csv("batch_comparison.csv")

with open("arch_hist.pickle", "rb") as f:
    hist_arch = pickle.load(f)[3:]

histories_batch = [histories[i].history for i in range(len(histories))]
with open("batch_hist.pickle", "wb") as f:
    pickle.dump(histories_batch, f)
histories3 = hist_arch + histories_batch
...
visualize2(histories3, labels=labels, type="accuracy", filename="batch_comparison",
          title="Comparison of accuracy on training set")
visualize2(histories3, labels=labels, type="loss", filename="batch_comparison",
          title="Comparison of loss on training set")
visualize2(histories3, labels=labels, type="val_accuracy", filename="batch_comparison",
          title="Comparison of accuracy on validation set")
visualize2(histories3, labels=labels, type="val_loss", filename="batch_comparison",
          title="Comparison of loss on validation set")

visualize2(histories3, labels=labels, type="accuracy", filename="batch_comparison2",
          title="Comparison of accuracy on training set", start_from=20)
visualize2(histories3, labels=labels, type="loss", filename="batch_comparison2",
          title="Comparison of loss on training set", start_from=20)
visualize2(histories3, labels=labels, type="val_accuracy", filename="batch_comparison2",
          title="Comparison of accuracy on validation set", start_from=20)
visualize2(histories3, labels=labels, type="val_loss", filename="batch_comparison2",
          title="Comparison of loss on validation set", start_from=20)

visualize2(histories3,labels=labels, type="lr", title="Changes of learning rate by callback",filename="batch_comparison")
